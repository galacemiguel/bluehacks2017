...
//        }
        ...
        let proj = Project(id: "0", imageURL: "hrllo", name: (self.form.rowBy(tag: "Name") as! TextRow).value!, holdingOffice: (self.form.rowBy(tag: "LocalGovernment") as! TextRow).value!, longitude: location.coordinate.longitude, latitude: location.coordinate.longitude, location: (self.form.rowBy(tag: "City") as! PushRow<String>).value!, contactNumber: (self.form.rowBy(tag: "ContactNumber") as! PhoneRow).value!, cost: (self.form.rowBy(tag: "Cost") as! DecimalRow).value!, dateStarted: (self.form.rowBy(tag: "DateStarted") as! DateRow).value!, dateExpectedCompletion: (self.form.rowBy(tag: "DateExpectedCompletion") as! DateRow).value!, projectDescription: (self.form.rowBy(tag: "ProjectDescription") as! TextAreaRow).value!, upvotes: 0, downvotes: 0)
